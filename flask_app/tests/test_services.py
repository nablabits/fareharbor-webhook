...
@patch("fh_webhook.model_services.CloseStoredRequest.run", return_value=True)
@patch("fh_webhook.services.ProcessJSONResponse.run")
@patch("fh_webhook.model_services.CreateStoredRequest.run")
def test_save_request_to_db(create_mock, json_mock, close_mock):
    timestamp = datetime.now(timezone.utc)
...
@patch("fh_webhook.services.ProcessJSONResponse.run")
@patch("fh_webhook.model_services.CreateStoredRequest.run")
def test_save_request_to_db_raises_error_for_close_stored_request(
    create_mock, json_mock
):
    timestamp = datetime.now(timezone.utc)
    unix_timestamp = str(timestamp.timestamp())
    filename = unix_timestamp + ".json"
